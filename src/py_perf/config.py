"""
Configuration management for PyPerf using OmegaConf.
...
- ./py-perf.yaml (current directory)
- ./.py-perf.yaml (current directory, hidden file)
- ~/.py-perf.yaml (home directory)
- ~/.config/py-perf/config.yaml (XDG config directory)
"""
...
class PyPerfConfig:
    """PyPerf configuration manager using OmegaConf."""
    ...
    def _load_user_config(self) -> Optional[DictConfig]:
        ...
        search_paths = [
            Path.cwd() / "py-perf.yaml",            # Current directory
            Path.cwd() / ".py-perf.yaml",           # Current directory (hidden)
            Path.home() / ".py-perf.yaml",          # Home directory
            Path.home() / ".config" / "py-perf" / "config.yaml",  # XDG config
        ]
    ...
    def is_enabled(self) -> bool:
        ...
        return self.get("py_perf.enabled", True)
    
    def is_debug(self) -> bool:
        """Check if debug mode is enabled."""
        return self.get("py_perf.debug", False)
...
_global_config: Optional[PyPerfConfig] = None


def get_config(config_override: Optional[Dict[str, Any]] = None) -> PyPerfConfig:
    """
    Get the global PyPerf configuration instance.
    
    Args:
        config_override: Optional runtime configuration overrides
        
    Returns:
        PyPerfConfig instance
    """
    global _global_config
    
    if _global_config is None or config_override:
        _global_config = PyPerfConfig(config_override)
    
    return _global_config


def reload_config(config_override: Optional[Dict[str, Any]] = None) -> PyPerfConfig:
    """
